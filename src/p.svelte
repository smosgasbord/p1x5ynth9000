<script>
  // ... (previous imports and variable declarations remain unchanged)

  let inputType = 'image'; // Change default to 'image'
  let imageLoaded = false; // New flag to check if an image has been loaded

  // ... (other variables and functions remain unchanged)

  function togglePlay() {
    if (inputType === 'image' && !imageLoaded) {
      showModal = true; // Show modal to select image if not loaded
    } else if (isPlaying) {
      pauseAudio();
      isPlaying = false;
    } else {
    ...
    if (inputType === 'image') {
      ...
      reader.onload = function(e) {
        ...
        img.onload = function() {
          ...
          cursorPosition = 0;
          imageLoaded = true; // Set flag when image is loaded
          showModal = false;
          startAudio(); // Start audio immediately after loading image
          isPlaying = true;
        }
  ...
  function handleInputChange(event) {
    ...
    imageLoaded = false; // Reset imageLoaded flag when input type changes
    if (inputType === 'image') {
      showModal = true;
    } else {
      updateInput();
  ...
  onMount(() => {
    ...
    // No need to call updateInput() here as we want to wait for user to select an image
  });
...
<script>
  ...
  for (let i = 0; i < controls.voiceCount; i++) {
    ...
    let x = Math.floor(cursorPosition);
    let y = Math.floor(Math.random() * sourceHeight);
    let index = (y * sourceWidth + x) * 4;
    let r = imageData.data[index] / 255;
    let g = imageData.data[index + 1] / 255;
    let b = imageData.data[index + 2] / 255;
